...
public abstract class Exporter {
   ...
   * load classes implementing the {@link PatientExporter} or
   * {@link PostCompletionExporter}
   ...
   * </pre>
   * 
   * The above define a single code map for ICD-10 codes and two code maps for CPT
   * codes.
   */
  ...
  public static class ExporterRuntimeOptions {
    ...
    /**
     * Register a new Flexporter mapping to be applied to Bundles from the FHIR
     * exporter.
  ...
  private static boolean exportRecord(Person person, String fileTag, long stopTime,
    ...
    if (options.terminologyService) {
      // Resolve any coded values within the record that are specified using a
      // ValueSet URI.
    ...
    if (Config.getAsBoolean("exporter.fhir.export")) {
      String runIdFolder = "/" + Generator.getRunID();
      File outDirectory = getOutputFolder("fhir" + runIdFolder, person);
  ...
  /**
   * Overwrite a file with the given contents. If the file doesn't exist it will
   * be created.
   * 
   * @param file     Path to the new file.
   * @param contents The contents of the file.
   ...
   * range falls
   * within the provided range or if `keepFunction` is provided, and returns
   * `true` for the given
   * entry.
   *
